...
function colorFamilyName(color) {
    ...
    const sectors = [
        [15, "red"], [45, "orange"], [75, "yellow"], [115, "lime"],
        [155, "green"], [195, "cyan"], [235, "azure"], [275, "blue"],
        [315, "violet"], [345, "magenta"], [360, "red"]
    ];
    for (const [limit, name] of sectors) if (h < limit) return name;
    return "red";
}
...
function isSameColorFamily(a, b) {
    ...
    return hueDistanceDegrees(pa.h, pb.h) <= SAME_HUE_DEGREES && Math.abs(pa.L - pb.L) <= 0.42;
}
...
function targetForRank(rank, top3, top10, normal) {
    return rank <= TOP3_COUNT ? top3 : rank <= TOP_COUNT ? top10 : normal;
}

function buildPairAdjacency() {
    const colorById = new Map(colors.map(color => [color.id, color]));
    const adjacency = new Map(colors.map(color => [color.id, []]));
    for (const [key, rawCount] of pairCounts.entries()) {
        const [aRaw, bRaw] = String(key).split("-");
        const a = colorById.get(Number(aRaw));
        const b = colorById.get(Number(bRaw));
        const count = Math.max(0, Number(rawCount) || 0);
        if (!a || !b) continue;
        adjacency.get(a.id).push({ opponent: b, count });
        adjacency.get(b.id).push({ opponent: a, count });
    }
    return adjacency;
}

function getColorDiversityInfo(color, rankMap = null, adjacency = null) {
    const ranks = rankMap ?? currentRankMap().map;
    const rank = ranks.get(color.id) ?? colors.length;
    const entries = (adjacency ?? buildPairAdjacency()).get(color.id) ?? [];
    ...
    for (const { opponent } of entries) {
        if (isSameColorFamily(color, opponent)) sameFamilyUnique++;
        else crossFamilyUnique++;
        if (isNearPerceptualColor(color, opponent)) nearColorUnique++;
        const opponentRank = ranks.get(opponent.id);
        if (Number.isFinite(opponentRank) && Math.abs(opponentRank - rank) <= NEAR_RANK_DISTANCE) nearRankUnique++;
        families.add(colorFamilyName(opponent));
    }

    const games = Math.max(Number(color.games) || 0, trackedGames);
    const trialTarget = targetForRank(rank, TRIAL_TARGET_TOP3, TRIAL_TARGET_TOP10, TRIAL_TARGET_NORMAL);
    const uniqueTarget = targetForRank(rank, UNIQUE_TARGET_TOP3, UNIQUE_TARGET_TOP10, UNIQUE_TARGET_NORMAL);
    const sameTargetRaw = targetForRank(rank, SAME_FAMILY_TARGET_TOP3, SAME_FAMILY_TARGET_TOP10, SAME_FAMILY_TARGET_NORMAL);
    const crossTargetRaw = targetForRank(rank, CROSS_FAMILY_TARGET_TOP3, CROSS_FAMILY_TARGET_TOP10, CROSS_FAMILY_TARGET_NORMAL);
    const nearRankTarget = targetForRank(rank, NEAR_RANK_TARGET_TOP3, NEAR_RANK_TARGET_TOP10, NEAR_RANK_TARGET_NORMAL);
    const availableSame = colors.reduce((count, candidate) => candidate !== color && isSameColorFamily(color, candidate) ? count + 1 : count, 0);
    ...
    const repeatQuality = games > 0 ? clamp(uniqueOpponents / games, 0, 1) : 0;
    const trialScore = clamp(gameProgress * 0.70 + uniqueProgress * 0.20 + repeatQuality * 0.10, 0, 1);
    ...
    const qualityScore = clamp(trialScore * 0.52 + diversityScore * 0.48, 0, 1);
    const uncertaintyMultiplier = 1 + (1 - diversityScore) * 0.55 + (1 - trialScore) * 0.30;

    return {
        rank, games, trialTarget,
        uniqueOpponents, uniqueTarget,
        sameFamilyUnique, sameTarget,
        crossFamilyUnique, crossTarget,
        nearColorUnique,
        nearRankUnique, nearRankTarget,
        familySpread: families.size,
        familySpreadTarget: FAMILY_SPREAD_TARGET,
        repeatQuality,
        gameProgress, uniqueProgress,
        sameFamilyProgress, crossFamilyProgress,
        nearRankProgress, familySpreadProgress,
        trialScore, diversityScore, qualityScore,
...
function buildAllColorDiversity(rankMap = null) {
    const ranks = rankMap ?? currentRankMap().map;
    const adjacency = buildPairAdjacency();
    const result = new Map();
    for (const color of colors) result.set(color.id, getColorDiversityInfo(color, ranks, adjacency));
    return result;
}

function diversityCoverageGate(diversityMap, pointSorted) {
    if (!pointSorted.length) return { gate: 0, allQuality: 0, allDiversity: 0, top10Quality: 0, top3Quality: 0 };
    ...
    const average = arr => arr.reduce((sum, value) => sum + value, 0) / Math.max(1, arr.length);
    ...
    return { gate, allQuality, allDiversity, top10Quality, top3Quality };
}
...
runMonteCarloAnalysis = function runMonteCarloAnalysisDiversityAware() {
    ...
    for (let sample = 0; sample < MONTE_CARLO_SAMPLES; sample++) {
        const simulated = colors.map(color => {
            ...
            return { color, score: color.mu + randomNormal() * color.sigma * diversity.uncertaintyMultiplier };
        });
        ...
        if (normalWithin >= normalNeeded && topOk && top3ExactOk) posteriorGoalSuccesses++;
    }
    ...
    for (const color of colors) {
        ...
        const current = pointRank.get(color.id);
        const diversity = diversityMap.get(color.id);
        const tolerance = current <= TOP3_COUNT ? TOP3_TOLERANCE : current <= TOP_COUNT ? TOP_TOLERANCE : NORMAL_TOLERANCE;
        let targetWithin = 0;
        let normalWithin = 0;
        let top10Within = 0;
        ...
        info.set(color.id, {
            currentRank: current, lowRank, highRank,
            confidence, normalConfidence, top10Confidence, tolerance,
            diversity,
    ...
    const top3ExactProbability = posteriorTop3ExactProbability * Math.min(1, coverage.top3Quality / 0.90);
    const avgConfidence = [...info.values()].reduce((sum, x) => sum + x.confidence, 0) / Math.max(1, colors.length);
    const normalTargetCount = Math.ceil(colors.length * REQUIRED_NORMAL_RATIO);

    analysisCache = {
        info, globalProbability, posteriorGoalProbability,
        averageConfidence: avgConfidence,
        normalTrusted, normalTargetCount, topTrusted, top3Trusted,
        top3ExactProbability, posteriorTop3ExactProbability,
        pointSorted, diversityMap,
...
function pairDiversityOpportunity(a, b, ai, bi) {
    ...
    return { gain, sameFamily, nearColor, newPair };
}

chooseBestPair = function chooseBestPairDiversityAware() {
    if (Math.min(...colors.map(c => c.games)) < BOOTSTRAP_GAMES) return chooseBootstrapPair();
    ...
    for (let i = 0; i < colors.length; i++) {
        ...
        for (let j = i + 1; j < colors.length; j++) {
            ...
            const opportunity = pairDiversityOpportunity(a, b, ai, bi);
            const da = ai.diversity;
            const db = bi.diversity;
            if (overlap === 0 && rankDistance > 10 && opportunity.gain < 1.25) continue;
            ...
            const targetWidthForRank = rank => rank <= TOP3_COUNT ? 1 : rank <= TOP_COUNT ? TOP_TOLERANCE * 2 + 1 : NORMAL_TOLERANCE * 2 + 1;
            const actualWidthA = ai.highRank - ai.lowRank + 1;
            const actualWidthB = bi.highRank - bi.lowRank + 1;
            const widthNeed =
                clamp(actualWidthA / targetWidthForRank(ai.currentRank) - 1, 0, 3) +
                clamp(actualWidthB / targetWidthForRank(bi.currentRank) - 1, 0, 3);
            const neighborBoost = rankDistance <= 1 ? 2.30 : rankDistance <= 3 ? 1.72 : rankDistance <= 6 ? 1.25 : rankDistance <= 10 ? 1.05 : 0.82;
            const proximity = Math.exp(-rankDistance / 14);
            const qualityNeed = (1 - da.qualityScore) + (1 - db.qualityScore);
            const familyBoost = opportunity.sameFamily && rankDistance <= 6 ? 1.24 : 1;
            const nearColorBoost = opportunity.nearColor && rankDistance <= 6 ? 1.18 : 1;
            const newOpponentBoost = opportunity.newPair ? 1.13 : 1;
            const top3ThreatBoost = (ai.lowRank <= TOP3_COUNT || bi.lowRank <= TOP3_COUNT)
                ? (Math.max(ai.currentRank, bi.currentRank) <= 5 ? 2.35 : 1.55)
                : 1;

            const score = (
                0.30 + entropy * 2.75 + overlap * 2.15 +
                (unresolvedA + unresolvedB) * 1.70 +
                widthNeed * 0.70 + qualityNeed * 1.05 + opportunity.gain * 0.92
            ) * importance * neighborBoost * familyBoost * nearColorBoost *
                newOpponentBoost * top3ThreatBoost * (0.45 + proximity * 0.55) *
                repeatPenalty + Math.random() * 0.003;
    ...
    for (let i = 0; i < sorted.length - 1; i++) {
        const a = sorted[i];
        const b = sorted[i + 1];
        const s = binaryEntropy(predictedProbability(a, b)) / (1 + getPairCount(a, b));
        if (s > fallbackScore) {
            fallbackScore = s;
            fallback = [a, b];
        }
...
if (stats && !document.getElementById("diversityAvg")) {
    ...
    stat.className = "stat";
    stat.innerHTML = '<span class="stat-label">平均多様性</span><strong id="diversityAvg" class="stat-value">0%</strong>';
    stats.appendChild(stat);
    const style = document.createElement("style");
    style.textContent = '@media (min-width:721px){.stats{grid-template-columns:repeat(6,minmax(0,1fr));}}';
    document.head.appendChild(style);
}

const goalNote = document.querySelector(".goal-note");
if (goalNote) {
    goalNote.textContent = "目標: 125色の90%以上が±5位以内、Top10が±2位以内、1〜3位の順番が完全一致する確率が90%以上。比較回数・ユニーク相手・同系色/異系色・近傍順位の多様性も信頼度と終了判定に反映します。Tierは表示のみです。";
}
...
renderStats = function renderStatsWithDiversity(forceAnalysis = false) {
    ...
    if (element) {
        ...
        element.title = `終了ゲート ${Math.round((analysis.diversityGate || 0) * 100)}% / Top10網羅度 ${Math.round((analysis.top10CoverageQuality || 0) * 100)}% / Top3網羅度 ${Math.round((analysis.top3CoverageQuality || 0) * 100)}%`;
    }
};
